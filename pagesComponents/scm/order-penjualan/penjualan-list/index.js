...
function OrderPenjualanList() {
    ...
    if (action === "delete") {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          try {
            const response = await deleteDataArrayOP({
  ...
  const CustomCell = ({ row, value }) => {
    ...
    const handleClick = () => {
      router.push(
        `/scm/order-penjualan/penjualan-view?id=${row.original.noReferensi}`
      );
    };
    ...
    const tooltipContent = (
      <div className={classes.tooltipContent}>
        <div className={classes.tooltipText}>
          <div>
            <strong>No Order:</strong> {row.original.idOrder}
          </div>
          <div>
            <strong>Total Harga:</strong> {row.original.harga}
          </div>
          <div>
            <strong>Total PPN:</strong> {row.original.ppn}
          </div>
          <div>
  ...
  const handleAdd = () => {
    setIsAdd(true);
    router.push("/scm/order-penjualan/penjualan-detail");
    setTimeout(() => {
  ...
  return (
    <Card>
      <MDBox p={3}>
        ...
        >
          <Grid item>
            <MDTypography variant="h5">List Order Penjualan</MDTypography>
          </Grid>
          {showActionControls && (
            <Grid item md={4}>
            ...
            <Autocomplete
              ...
              renderInput={(params) => (
                <TextField
                  ...
                  //   },
                  //   "& .MuiAutocomplete-tag": {
                  //     maxWidth: "calc(100% - 8px)", // To ensure tag is within the column
                  //     overflow: "hidden",
                  //     textOverflow: "ellipsis",
                  //     whiteSpace: "nowrap",
                  //   },
...
export default OrderPenjualanList;
